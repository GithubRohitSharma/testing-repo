...
router.post('/admin/upload-timetable', authFaculty, upload.single('fileInput'), async (req, res) => {
  try {
    const { body, file } = req;
...
router.post('/admin/upload-faculty', authFaculty, upload.single('photoInput'), async (req, res) => {
  try {
    const { body, file } = req;
